...
bass_manytags <- c('OPS-176', "OPS-224", 'OPS-177', "OPS-225")

detections_bass_manytags <- detections_bass %>%
  dplyr::filter(acoustic_tag_id %in% bass_manytags)


FISHINTEL_animal_proj <- etn::get_acoustic_projects(acoustic_project_code = "FISHINTEL")
