...
# Fetching all active deployments in the ws1 and ws2 projects

```{r get deployments, include=T, echo=T}
...
current_deployments$needs_battery_change <- NA
...
```{r receivermap, echo=FALSE}

receiver_map <- leaflet() %>% 
  addTiles() %>%
...
knitr::kable(current_deployments %>%
              dplyr::mutate(deploy_date_time = deploy_date_time %>% format("%Y-%m-%d"),
               ...
               dplyr::select(receiver_id, station_name, deploy_latitude, deploy_longitude,
                             battery_installation_date, battery_estimated_end_date,  deploy_date_time, needs_battery_change),

             booktabs = T, escape = F, format = "html") %>%
