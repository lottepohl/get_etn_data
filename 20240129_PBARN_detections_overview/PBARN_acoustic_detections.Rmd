---
title: "PBARN Detections and Institutes Overview"
output:
...
detections_2023 <- etn::get_acoustic_detections(con, start_date = "2023-01-01", acoustic_project_code = PBARN_acoustic_projects)


colnames(detections_2023)

animals_detected <- 
  detections_2023 %>%
    dplyr::select(tag_serial_number) %>% #, acoustic_tag_id, animal_id, animal_project_code
    unique() %>%
    tidyr::drop_na()
...
tags_2023 %>% colnames() #owner_organization is the column that I need!



#TODO: table with detection_id, date_time, tag_serial_number, acoustic_tag_id, animal_id, station_name, scientific_name, animal_project, owner_organization
...
current_deployments <- etn::get_acoustic_deployments(con, 
                                                  acoustic_project_code = c("ws1", "ws2", "cpodnetwork", "bpns", 'zeeschelde', "bovenschelde"), #"bpns", # c("ws1", "ws2")
                                                  open_only = TRUE) 
...
# # Apply factor() to selected columns using mutate()
...
This dashboard gives an overview on acoustic detections in the Permanent Belgian Acoustic Receiver Network (PBARN) between January 2023 and February 2024.

It will insights in which are the strategically most relevant PBARN receivers that need to be replaced with OP receivers in the coming weeks/months.

For this, information in several aspects will be given per receiver station:

* number of species

* number of individuals

* which institutes tagged these individuals
